...
bool MShader::AddUniform(const char* Name, const char* SearchName) {
	if(!ProgramId) {
		LogFile<<"Shader: Program not loaded"<<endl;
		return false;
	}
	if(!Name || !SearchName) {
		LogFile<<"Shader: Uniform name or uniform search name is not set: "<<SearchName<<endl;
		return false;
	}
	map<const char*, GLuint, stStringCompare>::iterator it;
	it = Uniforms.find(Name);
	if(it == Uniforms.end()) {
		GLuint Id = glGetUniformLocation(ProgramId, SearchName);
		if(Id == -1) {
			LogFile<<"Shader: Can't get uniform: "<<SearchName<<endl;
			return false;
		}
		Uniforms.insert(pair<const char*, GLuint>(Name, Id));
	}
	else LogFile<<"Shader: Uniform alredy loaded"<<endl;
